app.tools.fill = {
   ...
   fillAdjecentPixels(pos, color) {
      ...
      for(var positionName in positions) {
         ...
         if(position.x < 0 || position.x == app.image.width) continue
         if(position.y < 0 || position.y == app.image.height) continue
         var pixelAtAdj = app.image.pixels[position.x][position.y]
         if(this.colorsCloseEnough(color, pixelAtAdj.color)) {
            app.image.drawPixel(position.x, position.y, app.ui.pallet.color)
